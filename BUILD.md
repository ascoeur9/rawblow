...
### v0.5.8 (준비 중)
니콘 DSLR AF 표시·컬링 Undo 추가 + 리뷰 후속 수정(#74~#80). ※ Windows 실행 이슈(#79)는 별도 진행 중이며, 해결 후 함께 릴리즈 예정.
- **니콘 DSLR 153점 AF 오버레이(#80)** — D850/D5/D500 등 위상차(뷰파인더) AF의 측거점을 사진
  위에 표시. AFInfo2 V0101 + 153점 물리 그리드(9행 A–I × 17열, 실측 정규화 좌표)로, 그동안
  미러리스(Z)만 되던 AF 포인트를 DSLR로 확장. 오토에어리어·그룹·단일 측거 모드 모두 지원.
- **컬링 되돌리기/다시 실행(#78)** — 빠른 단축키 컬링 중 오조작을 Ctrl/⌘Z로 즉시 취소
  (Ctrl/⌘⇧Z·Ctrl/⌘Y = 다시 실행). 라벨·별점·태그를 최대 300단계 기록하고, 되돌리면 그 사진으로
  돌아가 바로 재평가. 단일·그리드 다중선택 모두 지원, 단축키 치트시트에 추가.
- **일시적 디코드 실패 고착 회복(#75)** — NAS 등 순간 끊김이 정상 파일을 영구 ⚠("열 수 없음")로
  고착시키던 문제(#64 후속) 수정: 성공 시 실패 카운터 리셋 + 임계 3→5 상향 + ⚠ 클릭 수동
  재시도(폴더 재열기 없이 회복).
- **자동소멸 토스트 클릭 통과(#76)** — 정보(3s)·공지(6s) 토스트가 떠 있는 동안 겹치는 우하단
  위젯 클릭을 삼키던 문제(#61 후속) 수정. 오류 토스트만 클릭으로 닫히고, 정보·공지는 포인터를
  통과시켜 작업 흐름을 끊지 않는다.
- **설정 화면 제목 전체 복원(#74)** — v0.5.7(#73)에서 "Settings"로 축약됐던 설정 화면 영문
  제목을 "Settings — Keyboard & General"로 복원(한국어·일본어도 각 언어 전체 제목).

### v0.5.7
UX 전수 리뷰 후속(2026-07-10) — 알림·데이터 안전·발견성 개선 13건(#61~#73).
- **토스트 알림 표시(#61)** — 안내·오류 메시지가 화면에 실제로 뜨도록 우하단 토스트 오버레이
  신설(심각도별 지속: 정보 3s / 공지 6s / 오류는 클릭까지 유지). 그간 설정만 되고 안 보이던
  안내가 전부 표면화.
- **셀렉 저장 실패 표면화 + 종료 플러시(#62)** — 사이드카 저장이 실패하면(읽기전용·권한·용량)
  데이터를 버리지 않고 재시도 + 상태바 "저장 실패"(원인·경로) + 오류 토스트. 종료 시
  사이드카·설정 동기 저장으로 디바운스 창 유실 방지.
- **원본 이동 확인 + 결과 정직성 + Enter(#63)** — 이동(Move) 전 확인 오버레이, 복사 성공·삭제
  실패 시 남은 원본 경고, 건너뜀 수 표시, 워커 비정상 종료 시 실패 리포트. Enter로 시작.
- **디코드 실패 상태(#64)** — 열 수 없는 파일은 무한 "디코딩 중…" 대신 3회 실패 후 ⚠ 표시
  (단일·그리드·필름스트립), 재시도·재페인트 루프 중단.
- **폴더 끝 도달 피드백(#65)** — 마지막 사진 너머 이동 시 "마지막 사진 · 미분류 N장" 토스트.
- **단축키 치트시트 오버레이(#66)** — ?(⇧/)·F1·툴바 ? 버튼으로 전체 단축키 표를 표시.
- **빈 화면 사유 구분 + 필터 초기화(#67)** — 빈 폴더와 "필터 결과 없음"을 구분하고 [필터 초기화]
  버튼 제공(단일·그리드·레일).
- **전송 대상 범위 선택(#68)** — 전송 다이얼로그에 "전체 / 현재 필터" 범위 세그먼트. 칩 카운트·
  플랜·리네임 미리보기·시작 수까지 선택 범위 기준으로 일관 계산.
- **설정 다듬기(#69)** — Esc로 닫기, 변경 즉시 저장, 기본값 복원(최근 폴더·전송 기본값은 보존),
  HEX 무효 입력 빨간 테두리, 캐시 경로 클릭 열기, 업데이트 배너 닫기 + 자동 확인 설정.
- **AI 컬링 기본/고급 접기(#70)** — 기본 모드(검사 4칩 + 결과·범위·시작) + "고급 옵션" 접기,
  사진가 친화 용어·툴팁, 접힌 채 고급 옵션 활성 시 경고.
- **모델 다운로드 모달(#71)** — 취소 버튼·Esc, 진행률·속도(MB/s) 표시, 오류 메시지 번역.
- **일관성 소소 묶음(#72)** — 결과 모달 배경 딤, 레일 행 포인터 커서, 툴바 툴팁.
- **i18n 마감(#73)** — 다이얼로그 캡션·레일 라벨의 하드코딩 영문을 번역 경유로 전환(한국어·일본어
  표시, 영어 UI는 무변화). 설정 화면 제목은 "Settings"로 단순화.

### v0.5.6
- **필터 뷰 이중 전진 수정(#55)** — 미선택 등 필터 뷰에서 평가(라벨·별점) 후 다음 사진을
  건너뛰고 다다음이 표시되던 버그 수정. 평가한 항목이 필터에서 빠지면 목록이 당겨지는 것을
  감안해 전진하지 않고 범위만 보정(픽/보류/제외·별점 필터에도 동일 적용).
- **촬영시간순 정렬(#56)** — 정렬 기준에 촬영시간순 추가, **기본값**. EXIF 촬영일시를
  백그라운드로 수집해 재정렬(UI·NAS 비차단), 동시각은 파일명 자연정렬·시각 없는 항목은 뒤로.
  설정 > 정렬 기준에서 파일명순으로 변경 가능.
- **다이얼로그 마지막 옵션 저장·복원(#57)** — 내보내기(전송)·폴더 정리 다이얼로그의 옵션
  (라벨/별점/태그, 복사·이동, 동반 범위, 분기, 충돌 정책, 리네임, 정리 기준)을 실행 시 저장하고
  다음 열기 때 기본값으로 복원. 대상 폴더는 저장하지 않고 매번 현재 폴더 기준 제안.
  (AI 컬링 옵션은 기존부터 저장·복원됨.)
- **CI 로컬 전환** — GitHub Actions 자동 실행 제거(수동 전용). 샘플 의존 테스트는 sample/
  부재 시 스킵되도록 정리, clippy -D warnings 통과.
