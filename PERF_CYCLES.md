...
## 부록 — 타 제조사 RAW 교차 검증 + SubIFD/체인 최적화 (공개 샘플 실측, 2026-06-02)
샘플 폴더엔 Canon CR2만 있어, 공개 샘플(raw.pixls.us/rawsamples.ch)을 받아 7개 제조사 포맷을 실측.
1차 검증에서 **전 포맷 정상 디코딩(패닉 0·에러 0)**을 확인했고, ARW/PEF/DNG 프리뷰가 임베디드를
IFD0 밖(**SubIFD 0x014A / IFD 체인 / JPEGInterchangeFormat 0x0201**)에 둬 전체파일을 읽는 비효율을 발견 →
IFD 스캔을 IFD0+체인+SubIFD 재귀로 확장(2-pass 헤더 확장, RW2/CR2는 추가 I/O 0)하고, ORIG/프리뷰
임베디드 선택을 **바이트 길이가 아닌 디코딩 해상도** 기준으로 바꿔(아래 회귀 가드) 최적화·적용.

**최적화 후 실측(preview=2048 타깃, ORIG=1:1):**
| 포맷(제조사) | thumb | preview (전→후) | ORIG | 비고 |
|---|---|---|---|---|
| **RW2(Panasonic, 사용자)** | 0.13MB✓ | 0.62MB/1920✓ | 6.52MB/8144✓ | 회귀 없음 |
| **CR2(Canon, 사용자)** | 0.13MB✓ | 2.28MB/2048✓ | **2.28MB/5616✓** | **ORIG 회귀 수정**(체인 스캔이 부른 1448 퇴화→5616 복원) |
| NEF(Nikon D70) | 0.13MB✓ | 1.68MB/2048✓ | 1.68MB/3008(33ms)✓ | ORIG 임베디드로 가속(153→33ms) |
| ARW(Sony A100) | 0.13MB✓ | 11.4MB/640(불가피) | 0.22MB/3881✓ | 카메라가 640 프리뷰만 내장(2006) → 한계 |
| RAF(Fuji X-T1) | 0.13MB✓ | 1.11MB/1920✓ | 0.07MB/4936✓ | 변화 없음(이미 최적) |
| ORF(Olympus E-M10) | 0.13MB✓ | 1.11MB/2048✓ | 0.07MB/4608✓ | 변화 없음 |
| **DNG(Pentax K-5)** | 0.13MB✓ | **20MB→1.28MB**/2047✓ | 1.28MB/4928(73ms)✓ | SubIFD 프리뷰 직독 |
| **PEF(Pentax K-x)** | 0.13MB✓ | **11.6MB→1.42MB**/2048✓ | 1.42MB/4288(60ms)✓ | IFD 체인 0x0201 프리뷰 직독 |
| SRW(Samsung NX500) | 0.13MB✓ | 1.07MB/2048✓ | 1.07MB/6480(106ms)✓ | ORIG 임베디드로 가속(506→106ms) |
- **결론: 9개 포맷(사용자 RW2/CR2 포함) 전부 thumb·preview·ORIG 정상.** DNG/PEF 프리뷰 I/O를 **8~16배** 줄이고
  NEF/SRW ORIG를 임베디드 직독으로 가속. **사용자 포맷(RW2/CR2) 회귀 0**(CR2 ORIG 5616 복원).
- **회귀 가드(핵심)**: SubIFD/체인 스캔을 켜면 본 프리뷰보다 바이트가 큰 저해상 JPEG가 후보에 섞여(CR2 ORIG가
  5616→1448로 퇴화) → ORIG/프리뷰 선택을 **해상도 기준**으로 변경(`decode_ifd_embedded`가 각 후보의 SOF를
  64KB head probe로 읽어 긴변 크기 비교). 무손실(SOF3) 등 디코딩 불가 JPEG은 `jpeg_dimensions`가 None → 자동 제외.
  합성 테스트 `orig_ifd_picks_by_resolution_not_byte_length`로 영구 가드.
- ARW 프리뷰(640)는 카메라가 그 이상 임베디드를 안 담아 불가피(전체파일 폴백으로 640 추출, 동작 정상).
- 합성 raw-only DNG(임베디드 프리뷰 없음)는 디코드 에러로 정상 처리(크래시 아님) — 실제 카메라 DNG는 정상.
